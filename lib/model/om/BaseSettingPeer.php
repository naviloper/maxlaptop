...
abstract class BaseSettingPeer {
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseSettingPeer', $criteria, $con);
		}
	...
	{
		return $withPrefix ? SettingPeer::CLASS_DEFAULT : SettingPeer::OM_CLASS;
	}
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseSettingPeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseSettingPeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseSettingPeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseSettingPeer', $values, $con, $pk);
    }
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseSettingPeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseSettingPeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseSettingPeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseSettingPeer', $values, $con, $ret);
    }
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseSettingPeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
