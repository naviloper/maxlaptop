...
abstract class BaseMenuItem extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the list_order field.
	 * Note: this column has a database default value of: '999999'
	 * @var        string
	 */
	...
	/**
	 * Get the [list_order] column value.
	 * 
	 * @return     string
	 */
	...
	public function setType($v)
	{
		...
		if ($this->type !== $v) {
			$this->type = $v;
			$this->modifiedColumns[] = MenuItemPeer::TYPE;
		}
	...
	public function setRefId($v)
	{
		...
		if ($this->ref_id !== $v) {
			$this->ref_id = $v;
			$this->modifiedColumns[] = MenuItemPeer::REF_ID;
		}
	...
	/**
	 * Set the value of [list_order] column.
	 * 
	 * @param      string $v new value
	 * @return     MenuItem The current object (for fluent API support)
	 */
	public function setListOrder($v)
	{
		...
		if ($this->list_order !== $v || $this->isNew()) {
			$this->list_order = $v;
			$this->modifiedColumns[] = MenuItemPeer::LIST_ORDER;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->menu_id = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->type = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->ref_id = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->list_order = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			...
			return $startcol + 7; // 7 = MenuItemPeer::NUM_COLUMNS - MenuItemPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseMenuItem:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseMenuItem:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseMenuItem:save:pre') as $callable)
			{
			...
			if ($this->isModified() && !$this->isColumnModified(MenuItemPeer::UPDATED_AT))
			{
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(MenuItemPeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseMenuItem:save:post') as $callable)
				{
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[4], $arr)) $this->setListOrder($arr[$keys[4]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(MenuItemPeer::TYPE)) $criteria->add(MenuItemPeer::TYPE, $this->type);
		if ($this->isColumnModified(MenuItemPeer::REF_ID)) $criteria->add(MenuItemPeer::REF_ID, $this->ref_id);
		if ($this->isColumnModified(MenuItemPeer::LIST_ORDER)) $criteria->add(MenuItemPeer::LIST_ORDER, $this->list_order);
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseMenuItem:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseMenuItem::%s', $method));
	  }
