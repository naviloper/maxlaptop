...
/**
 * Base class that represents a row from the 'shop_vitrin' table.
 *
 * 
 *
 ...
 */
abstract class BaseShopVitrin extends BaseObject  implements Persistent {
	...
	public function setShopId($v)
	{
		...
		if ($this->shop_id !== $v) {
			$this->shop_id = $v;
			$this->modifiedColumns[] = ShopVitrinPeer::SHOP_ID;
		}
	...
	public function setConfigId($v)
	{
		...
		if ($this->config_id !== $v) {
			$this->config_id = $v;
			$this->modifiedColumns[] = ShopVitrinPeer::CONFIG_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->shop_id = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			...
			return $startcol + 3; // 3 = ShopVitrinPeer::NUM_COLUMNS - ShopVitrinPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating ShopVitrin object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ShopVitrinPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseShopVitrin:delete:pre') as $callable)
			{
			...
			if ($ret) {
				ShopVitrinPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseShopVitrin:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseShopVitrin:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseShopVitrin:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				ShopVitrinPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = ShopVitrinPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = ShopVitrinPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = ShopVitrinPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ShopVitrinPeer::ID)) $criteria->add(ShopVitrinPeer::ID, $this->id);
		if ($this->isColumnModified(ShopVitrinPeer::SHOP_ID)) $criteria->add(ShopVitrinPeer::SHOP_ID, $this->shop_id);
		if ($this->isColumnModified(ShopVitrinPeer::CONFIG_ID)) $criteria->add(ShopVitrinPeer::CONFIG_ID, $this->config_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(ShopVitrinPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of ShopVitrin (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setShopId($this->shop_id);
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     ShopVitrin Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     ShopVitrinPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new ShopVitrinPeer();
		}
		return self::$peer;
	}
	...
	public function getConfig(PropelPDO $con = null)
	{
		if ($this->aConfig === null && (($this->config_id !== "" && $this->config_id !== null))) {
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aConfig->addShopVitrins($this);
			 */
		}
		return $this->aConfig;
	}
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseShopVitrin:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseShopVitrin::%s', $method));
	  }
